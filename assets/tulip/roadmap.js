...
var helpText =
  ...
  'JunctionType: is one of R, M, J, T, r, m, j, t.<br>'+
  'R,r = Roundabout, M,m = Mini-roundabout, J,j = Junction, T,t = Traffic lights.<br>'+
  'Exit: The primary exit to draw.<br>'+
  'For uppercase JunctionTypes, Exit and Other exits must be numbers from 0-12.(bearing by clock position)<br>'+
  'For lowercase JunctionTypes, Exit and Other exits must be numbers from 0-360.(bearing by degrees)<br>'+
...
function AddWaypoint(){
  ...
  var InstructionID = form.elements.InstructionID.value.trim();
  var inputs = [interval, [JunctionType , exit , others], description];
  if (InstructionID === "") {
...
function tulip_gen(input) {
  ...
  var errors = [];
  if (!/^[RrMmJjTt]$/.test(JunctionType)) {
    errors.push("JunctionType must be one of R, M, J, T, r, m, j, t.");
  }

  var minExit = 0;
  var maxExit = JunctionType === JunctionType.toUpperCase() ? 12 : 360;
  ...
  if (JunctionType === "R" || JunctionType === "M" || JunctionType === "J" || JunctionType === "T") {
